...
class RoutineRepository extends BaseRepository {
  ...
  Future<String> addRoutineExercise(
    ...
    final count =
        Sqflite.firstIntValue(
          await db.rawQuery(
            'SELECT COUNT(*) FROM routine_exercises WHERE routine_day_id = ?',
            [routineDayId],
          ),
        ) ??
        0;
  ...
  Future<String> addPredefinedSet(
    ...
    final count =
        Sqflite.firstIntValue(
          await db.rawQuery(
            'SELECT COUNT(*) FROM predefined_sets WHERE routine_exercise_id = ?',
            [routineExerciseId],
          ),
        ) ??
        0;
  ...
  Future<void> updatePredefinedSet(
    ...
    final updates = <String, dynamic>{};
    if (weight != null) {
      updates['weight'] = _normalizeRoutineDecimal(weight, 2);
    }
    if (reps != null) updates['reps'] = reps;
    if (distance != null) {
      updates['distance'] = _normalizeRoutineDecimal(distance, 2);
    }
