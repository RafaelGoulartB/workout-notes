...
class TestDataGenerator {
  ...
  /// Returns counts for workouts, routines, and sleep entries.
  Future<Map<String, int>> generate({int months = 4}) async {
    ...
    if (exercises.isEmpty) {
      final sleepCount = await _generateSleepEntries(periodStart, now);
      return {'workouts': 0, 'routines': 0, 'sleep': sleepCount};
    }
    ...
    // Generate nightly sleep entries over the same period
    final sleepCount = await _generateSleepEntries(periodStart, now);
  ...
  /// Generates nightly sleep data with occasional missing actual-sleep values
  /// so the dashboard can be previewed in both complete and partial states.
  Future<int> _generateSleepEntries(DateTime periodStart, DateTime now) async {
    final db = await _db.database;
    final totalDays = now.difference(periodStart).inDays;
    var created = 0;

    for (var dayOffset = 0; dayOffset <= totalDays; dayOffset++) {
      // Keep a few gaps so the consistency metric is meaningful.
      if (_rng.nextDouble() < 0.08) continue;

      final date = periodStart.add(Duration(days: dayOffset));
      final dateStr = date.toIso8601String().substring(0, 10);
      final trend = math.sin(dayOffset / 12) * 18;
      final sleepMinutes = (450 + trend + _rng.nextInt(71) - 35).round().clamp(
        300,
        600,
      );
      final hasActual = _rng.nextDouble() >= 0.22;
      final actualSleepMinutes = hasActual
          ? (sleepMinutes - (8 + _rng.nextInt(53))).clamp(240, sleepMinutes)
          : null;
      final bedtimeMinutes = 21 * 60 + _rng.nextInt(180);
      final wakeTimeMinutes =
          (bedtimeMinutes + sleepMinutes + _rng.nextInt(50)) % (24 * 60);
